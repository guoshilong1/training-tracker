...
// ---------- 区域 ID → 显示名称（与前端 index.html 的 AREA_CONFIG 保持一致，去掉"带训追踪系统"后缀） ----------
// 优先从同目录 index.html 的 AREA_CONFIG 提取，保证与前端完全一致、不漂移；
// 提取失败时回退到内置常量 AREA_NAMES。
const AREA_NAMES = {
  '666': '白高朋大区',
  '000': '郭士龙区域',
  '001': '李澳区域',
...
// 从 index.html 的 AREA_CONFIG 动态提取 id → 显示名（去掉"带训追踪系统"后缀），保证与前端零漂移
function loadAreaNames() {
  try {
    const html = fs.readFileSync(path.join(__dirname, 'index.html'), 'utf8');
    const map = {};
    const re = /\{\s*id:\s*'([^']+)'[\s\S]*?name:\s*'([^']*)带训追踪系统'/g;
    let m;
    while ((m = re.exec(html))) {
      const id = m[1];
      const full = m[2];
      if (full) map[id] = full.length ? full : map[id]; // full = 去掉后缀前的名字主体
    }
    if (Object.keys(map).length > 0) return map;
  } catch (e) {
    console.log('从 index.html 提取区域名失败，使用内置映射:', e.message);
  }
  return AREA_NAMES;
}

// 缓存提取结果，避免每次调用都读文件
let _areaNameCache = null;
function areaDisplayName(id) {
  if (!_areaNameCache) _areaNameCache = loadAreaNames();
  return _areaNameCache[id] || id; // 未知区域回退显示原始 id
}
